...
class Oeditor_Ordereditor_Adminhtml_Sales_Order_EditController extends Mage_Adminhtml_Sales_Order_EditController
{
 ...
 protected function _isAllowed()
    ...
    public function saveAction()
	{
		 //echo '<pre>';print_r($_POST);die;
			  if(isset($_POST['edit_order_number']) && $_POST['edit_order_number'] != "")
			 {
				...
				/**********Remove Items Start**********/
					$removeItemsData = $this->removeItems($order,$logEnabled,$writeConnection,$connectionRead,$manage_inventory,$lockField);
					$removeItemArray = $removeItemsData['remove_item_array'];
				/**********Remove Item(s) Ends**********/


				/*Re-add items those were deleted above STARTS*/
	$itemAddData = $this->addNewItems($oldOrder,$quote,$manage_inventory,$logEnabled,$itemImgToLog,$removeItemArray,$writeConnection,$connectionRead,$currencyCode,$lockField);
				...
				/*Update Shipping Data -  same as billing STARTS*/
					if(isset($postShippingFormFields) && is_array($postShippingFormFields) && $postOrder['has_shipping'] != "")
					{
				...
				$b = $address->collectTotals()->getData();
				$oldOrder->setData('base_subtotal_incl_tax',$b['subtotal_incl_tax'] );
				$oldOrder->setData('subtotal_incl_tax',$b['subtotal_incl_tax']);
				
				
				$address15 = $quote->getShippingAddress();
				$d15 = $address15->getData();
				
				$oldOrder->setData('shipping_amount',$d15['shipping_amount']);
				$oldOrder->setData('base_shipping_amount',$d15['base_shipping_amount']);
				
				$oldOrder->setData('shipping_tax_amount',$d15['shipping_tax_amount']);
				$oldOrder->setData('base_shipping_tax_amount',$d15['base_shipping_tax_amount']);
				
				$oldOrder->setData('base_shipping_incl_tax',$d15['base_shipping_incl_tax']);
				$oldOrder->setData('shipping_incl_tax',$d15['shipping_incl_tax']);
				
				/********************updated 22-4-15 Ends *********************/
			...
			//	$oldOrder->save();
				...
				if(isset($_POST['order']['send_confirmation']) && $_POST['order']['send_confirmation'] == 1)
				{
					$oldOrder->sendNewOrderEmail();
					$historyItem = Mage::getResourceModel('sales/order_status_history_collection')
						->getUnnotifiedForInstance($oldOrder, Mage_Sales_Model_Order::HISTORY_ENTITY_NAME);
					if ($historyItem) {
	...
	public function removeItems($order,$logEnabled,$writeConnection,$connectionRead,$manage_inventory,$lockField)
	{
	...
	if ($itemStatusId == 1 || $itemStatusId == 3) { /* skip the item from being delete those are alredy invoiced*/
	...
	public function addNewItems($oldOrder,$quote,$manage_inventory,$logEnabled,$itemImgToLog,$removeItemArray,$writeConnection,$connectionRead,$currencyCode,$lockField)
	{
